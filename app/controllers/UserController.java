...
public class UserController extends Controller {
	...
	public Result retrieveProductsByUser(Long userId) {
		
		User user = User.findById(userId);
		List<Product> userProductList =user.getProducts();
		...
		if (user == null) {
		 ...
		 if (request().accepts("application/json")) {
				JsonNode node = Product.toJsonArray(userProductList);
				return ok(node);
		}	
		 
		 else if (request().accepts("application/xml")) {
				return ok(views.xml.products.render(userProductList));
		}
